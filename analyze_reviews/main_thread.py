...
def main(project, code_review_partition, i):
...
if __name__ == "__main__":
    ...
    for project in all_crop_projects:
        ...
        code_review_partition_list = partition(list(review_by_revision.keys()), 10)
    
    threads = list()
    for i, code_review_partition in enumerate(code_review_partition_list):
        logging.info(f"Main    : create and start thread {i}.")
        x = threading.Thread(target=main, args=(project,code_review_partition, i))
        threads.append(x)
        x.start()
